...
async def analytics_dashboard(request: Request) -> HTMLResponse:
    ...
    try:
        ...
        total_submissions = (
            db.query(func.sum(VerifiedDomain.submission_count)).scalar() or 0
        )
        verified_domains = (
            db.query(VerifiedDomain).filter(VerifiedDomain.verified).count()
        )
        ...
        active_last_24h = (
            ...
            .filter(VerifiedDomain.last_submission_at >= twenty_four_hours_ago)
            .count()
        )

        active_last_7d = (
            db.query(VerifiedDomain)
            .filter(VerifiedDomain.last_submission_at >= seven_days_ago)
            .count()
        )
        ...
        top_domains = (
            db.query(VerifiedDomain)
            .order_by(desc(VerifiedDomain.submission_count))
            .limit(10)
            .all()
        )
        ...
        recent_domains = (
            db.query(VerifiedDomain)
            .filter(VerifiedDomain.last_submission_at >= month_ago)
            .order_by(desc(VerifiedDomain.last_submission_at))
            .limit(10)
            .all()
        )
        ...
        for domain in recent_domains:
            ...
            has_activity_today = (
                domain.last_submission_at and domain.last_submission_at >= today_start
            )
            has_activity_week = (
                domain.last_submission_at and domain.last_submission_at >= week_ago
            )
            has_activity_month = (
                domain.last_submission_at and domain.last_submission_at >= month_ago
            )
        ...
        high_volume_domains = (
            db.query(VerifiedDomain)
            .filter(VerifiedDomain.submission_count >= 50)
            .count()
        )
        ...
        rate_limit_risk = (
            db.query(VerifiedDomain)
            .filter(VerifiedDomain.submission_count >= 8)
            .count()
        )
        ...
        email_recipients = (
            ...
            .group_by(VerifiedDomain.recipient_email)
            .order_by(desc("total_emails"))
            .limit(10)
            .all()
        )
        ...
        stats = {
            ...
            "average_submissions_per_domain": (
                round(total_submissions / total_domains, 2) if total_domains > 0 else 0
            ),
